...
// Dragon Flap · the whole game. Built by the Core builder, following CONTRACT.md section 4.
...
(function () {
  ...
  // Placeholder drawings: plain colored rectangles, same inputs as the Art table.
  ...
  // ----- Plays differently: easy-mode buttons -----
  ...
  function frame(now) {
    let seconds = 0;
    if (lastTime !== null) seconds = Math.min((now - lastTime) / 1000, 0.05);
    lastTime = now;
    elapsed += seconds;
    ...
    if (state === 'playing') {
      ...
      const top = y - CONFIG.birdSize / 2;
      const bottom = y + CONFIG.birdSize / 2;
      const left = birdX - CONFIG.birdSize / 2;
      const right = birdX + CONFIG.birdSize / 2;
      let hit = bottom >= CONFIG.canvasHeight - CONFIG.groundHeight || top <= 0;
      if (!hit) {
        for (const pipe of pipes) {
          if (right > pipe.x && left < pipe.x + CONFIG.pipeWidth && (top < pipe.gapTop || bottom > pipe.gapBottom)) { hit = true; break; }
        }
      }
      if (hit) crash();
    ...
    // Drawing order: background, pipes, ground, bird, then the score.
    const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    drawBackground(ctx, CONFIG.canvasWidth, CONFIG.canvasHeight, reduceMotion ? 0 : elapsed);
    for (const pipe of pipes) {
    ...
    if (state === 'playing') {
      ...
      ctx.strokeText(String(score), CONFIG.canvasWidth / 2, 20);
      ctx.fillStyle = '#ffffff';
      ctx.fillText(String(score), CONFIG.canvasWidth / 2, 20);
      ctx.restore();
    }
